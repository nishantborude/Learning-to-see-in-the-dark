...
for epoch in range(lastepoch,4001):
    ...
    for ind in np.random.permutation(len(train_ids)):
        ...
        if input_images[str(ratio)[0:3]][ind] is None:
            ...
            im = gt_raw.postprocess(use_camera_wb=True, no_auto_bright=True)
            gt_images[ind] = np.expand_dims(np.float32(im)/255,axis = 0)
        ...
        # print(np.min(input_patch), np.max(input_patch))
